...
class ImageManager
{
	...
	function processUploads() 
	{
		...
		if(isset($_FILES['upload']['tmp_name']) && $this->validRelativePath($relative))
		{		
	...
	function _processFiles($relative, $file)
	{
		...
		{
			Files::delFile($file['tmp_name']);
            chmod($path.$file['name'],0666);
            $bw=(preg_match("/^[0-9]+$/",$_REQUEST['bw']))?$_REQUEST['bw']:0;
            $bh=(preg_match("/^[0-9]+$/",$_REQUEST['bh']))?$_REQUEST['bh']:0;
            if($bw || $bh){
              $img = Image_Transform::factory(IMAGE_CLASS);
  		      $img->load($path.$file['name']);
              if($bw)$img->scaleMaxX($_REQUEST['bw']);
              else $img->scaleMaxY($_REQUEST['bh']);
              $img->save($path.$file['name']);
  		      $img->free();
              chmod($path.$file['name'],0666);
            }
